  ...
  return (
    <div>
      ...
      >
        ...
        <span className={`font-mono text-xs truncate flex-1 ${node.selfSize > 0 ? "text-indigo-400" : "text-gray-300"}`}>
          {fn}
        </span>
        {url && (
        ...
        {node.selfSize > 0 && (
          <span className="text-xs text-gray-400 whitespace-nowrap">{formatBytes(node.selfSize)}</span>
        )}
        {pct >= 1 && (
      ...
      {expanded &&
        node.children.map((child, i) => (
          <TreeRow key={`${depth}-${i}-${child.callFrame.functionName}`} node={child} totalSize={totalSize} depth={depth + 1} />
        ))}
    </div>
  );
}
