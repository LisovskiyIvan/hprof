...
export default function Timeline({ base }: { base: string }) {
  ...
  useEffect(() => {
    ...
    for (const entry of entries) {
      const idx = Math.min(Math.floor((entry.timestamp - minTime) / bucketSize), bucketCount - 1)
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-6 text-sm text-gray-400">
        <span>
          Entries: <span className="text-white">{data.timeline.length.toLocaleString()}</span>
        </span>
        <span>
          Allocated: <span className="text-indigo-400">{formatBytes(totalAlloc)}</span>
        </span>
        <span>
          Freed: <span className="text-red-400">{formatBytes(totalFreed)}</span>
        </span>
      </div>
