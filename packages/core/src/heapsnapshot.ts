...
export class HeapSnapshot {
          ...
          if (objectStart !== -1 && objectStart < nodesIndex) {
            ...
            for (let i = objectStart; i < prefix.length; i++) {
              ...
              if (ch === '{') {
                ...
                depth -= 1
                if (depth === 0) {
                  return JSON.parse(prefix.slice(objectStart, i + 1)) as HeapSnapshotMeta
                }
              }
            }
          }
  ...
  async streamSummary(options?: {
    ...
    let mode: 'seekNodes' | 'parseNodes' | 'seekStrings' | 'parseStrings' | 'done' = 'seekNodes'
    let record: number[] = []
    let currentNumber = ''
    let currentString = ''
    ...
    for await (const chunk of stream) {
      ...
      if (onProgress && pct !== lastProgressPct) {
        lastProgressPct = pct
        const phase =
          mode === 'seekNodes' || mode === 'parseNodes'
            ? 'nodes'
            : mode === 'seekStrings' || mode === 'parseStrings'
              ? 'strings'
              : 'done'
  ...
  private createNodeFromRaw(raw: HeapSnapshotRawData, nodeIndex: number): HeapSnapshotNode {
    ...
    return {
      type:
        raw.nodeTypes[raw.nodes[base + raw.nodeOffsets.type]!] ??
        String(raw.nodes[base + raw.nodeOffsets.type]),
      name:
        raw.strings[raw.nodes[base + raw.nodeOffsets.name]!] ??
        `<string#${raw.nodes[base + raw.nodeOffsets.name]}>`,
  ...
  async getNodeEdges(
    ...
    for (let edgeIndex = edgeStart; edgeIndex < edgeEnd; edgeIndex++) {
      ...
      const edgeType =
        raw.edgeTypes[raw.edges[base + raw.edgeOffsets.type]!] ??
        String(raw.edges[base + raw.edgeOffsets.type])
      const nameOrIndexValue = raw.edges[base + raw.edgeOffsets.nameOrIndex]!
      edges.push({
        type: edgeType,
        nameOrIndex:
          edgeType === 'element' || typeof nameOrIndexValue === 'number'
            ? nameOrIndexValue
  ...
  private parseFull(): HeapSnapshotResult {
    ...
    for (let base = 0; base < raw.nodes.length; base += nodeFieldCount) {
      nodes.push({
        ...
        edgeCount: raw.nodes[base + nodeEdgeCountIdx]!,
      })
    }

    const edges: HeapSnapshotEdge[] = []
    for (let base = 0; base < raw.edges.length; base += edgeFieldCount) {
      const nameOrIndexVal = raw.edges[base + edgeNameOrIndexIdx]!
      const edgeType =
        edgeTypes[raw.edges[base + edgeTypeIdx]!] ?? String(raw.edges[base + edgeTypeIdx])
      edges.push({
        type: edgeType,
        nameOrIndex:
          edgeType === 'element' || typeof nameOrIndexVal === 'number'
            ? nameOrIndexVal
            : (raw.strings[nameOrIndexVal] ?? String(nameOrIndexVal)),
        toNode: Math.floor(raw.edges[base + edgeToNodeIdx]! / nodeFieldCount),
      })
    }
  ...
  private buildRetainedSize(): number[] {
    ...
    while (stack.length > 0) {
      ...
      visited[nodeIdx] = 1
      stack.push(~nodeIdx)
      const edgeStart = edgeStarts[nodeIdx]!
      const edgeEnd = edgeStarts[nodeIdx + 1]!
      for (let edgeIndex = edgeStart; edgeIndex < edgeEnd; edgeIndex++) {
        const toNode = edges[edgeIndex]!.toNode
    ...
    for (let n = 0; n < nodeCount; n++) {
      const edgeStart = edgeStarts[n]!
      const edgeEnd = edgeStarts[n + 1]!
      for (let edgeIndex = edgeStart; edgeIndex < edgeEnd; edgeIndex++) {
        const toNode = edges[edgeIndex]!.toNode
