...
function getLibFilename(): string {
  switch (process.platform) {
    case "darwin": return "libhprof_c.dylib";
    case "win32": return "hprof_c.dll";
    default: return "libhprof_c.so";
  }
}
...
function getNpmPackageName(): string {
  ...
  if (platform === "linux") return `@hprof/hprof-bin-linux-${arch === "arm64" ? "arm64" : "x64"}-gnu`;
...
const ffiDefinition = {
  ...
  hprof_snapshot_summary: { args: [FFIType.ptr, FFIType.u32, FFIType.cstring], returns: FFIType.ptr },
  hprof_snapshot_node_page: { args: [FFIType.ptr, FFIType.u32, FFIType.u32, FFIType.cstring, FFIType.cstring, FFIType.u8, FFIType.u8], returns: FFIType.ptr },
  ...
  hprof_profile_summarize: { args: [FFIType.ptr, FFIType.u32, FFIType.cstring], returns: FFIType.ptr },
  ...
  hprof_timeline_summary: { args: [FFIType.ptr, FFIType.u32, FFIType.cstring], returns: FFIType.ptr },
...
function loadLib(): Lib {
  ...
  if (!binaryPath) throw new Error("hprof native library not found. Build from source with `cargo build --release -p hprof-c`");
...
function readResult(p: Pointer | null): { ok: true; handle: number } | { ok: false; error: string } {
...
export function snapshotSummary(handle: NativeHandle, top = 30, filter?: string): any {
  return JSON.parse(callString(loadLib().symbols.hprof_snapshot_summary(handle, top, ptr(encode(filter ?? "")))));
}

export function snapshotNodePage(handle: NativeHandle, options: {
  page?: number; pageSize?: number; type?: string | null; q?: string | null;
  sort?: "id" | "type" | "name" | "selfSize" | "edgeCount"; dir?: "asc" | "desc";
} = {}): any {
  ...
  return JSON.parse(callString(loadLib().symbols.hprof_snapshot_node_page(
    handle, options.page ?? 0, options.pageSize ?? 100,
    ptr(encode(options.type ?? "")), ptr(encode(options.q ?? "")),
    sortVal, dirVal,
  )));
}
...
export function profileSummarize(handle: NativeHandle, top?: number, filter?: string): any {
  return JSON.parse(callString(loadLib().symbols.hprof_profile_summarize(handle, top ?? 0, ptr(encode(filter ?? "")))));
}
...
export function timelineSummary(handle: NativeHandle, top?: number, filter?: string): any {
  return JSON.parse(callString(loadLib().symbols.hprof_timeline_summary(handle, top ?? 0, ptr(encode(filter ?? "")))));
}
...
export function isAvailable(): boolean {
  try { loadLib(); return true; } catch { return false; }
}
